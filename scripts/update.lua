...
local function GetGrandLeaderOrOwner(ent, visited)
    visited = visited or {}
    ...
    local leader = ent.components.follower ~= nil and ent.components.follower.leader or nil
    local owner = ent.components.inventoryitem ~= nil and ent.components.inventoryitem:GetGrandOwner() or nil

    local leader_or_owner = leader or owner

    if leader_or_owner ~= nil and
        (leader_or_owner.components.inventoryitem ~= nil or leader_or_owner.components.follower ~= nil)
    then
        return GetGrandLeaderOrOwner(leader_or_owner, visited)
    end

    return leader_or_owner
end
...
function LongUpdate(dt, ignore_player)
    ...
    if player ~= nil and ignore_player then
        if player.components.beard ~= nil then
            player.components.beard.pause = true
        end

        if player.components.beaverness ~= nil then
    ...
    for i, ent in pairs(Ents) do
        ...
        if ignore_player then
            if ent.components.inventoryitem then
                if GetGrandLeaderOrOwner(ent) == player then
                    should_ignore = true
                end
            end
